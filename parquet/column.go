...
type BooleanColumnChunkReader struct {
	...
	// decoders
	decoder  booleanPlainDecoder // TODO: maybe pointer?
	dDecoder *rleDecoder
	rDecoder *rleDecoder
}
...
func NewBooleanColumnChunkReader(r io.ReadSeeker, schema Schema, chunk *parquetformat.ColumnChunk) (*BooleanColumnChunkReader, error) {
	...
	cr.maxD, cr.maxR = schema.maxLevels(meta.PathInSchema)
	...
	if *schemaElement.RepetitionType == parquetformat.FieldRepetitionType_REQUIRED {
		...
		cr.dDecoder = newRLEDecoder(bitWidth(cr.maxD))
	}
	if cr.curD == 0 && *schemaElement.RepetitionType != parquetformat.FieldRepetitionType_REPEATED {
		...
		cr.rDecoder = newRLEDecoder(bitWidth(cr.maxR))
	}
...
func (cr *BooleanColumnChunkReader) Next() bool {
	...
	if cr.decoder.data == nil {
		...
		cr.decoder.init(data[start:], cr.header.DataPageHeader.NumValues)
	...
	if cr.dDecoder != nil {
		cr.curD = int(cr.dDecoder.nextInt32())
	}
	if cr.rDecoder != nil {
		cr.curR = int(cr.rDecoder.nextInt32())
	}
	if cr.curD == cr.maxD {
		if !cr.decoder.next() {
			cr.err = fmt.Errorf("not enough data")
			return false
		}
...
// TODO: consider one method for D and R returning sruct {D, R int}
